...
where gh >nul 2>nul
if errorlevel 1 (
  echo [ERROR] GitHub CLI (gh) not found. Install gh first.
  exit /b 1
)
...
if errorlevel 1 (
  echo [ERROR] No default repository. Run in repo root or: gh repo set-default ^<ORG/REPO^>
  exit /b 1
)

powershell -NoProfile -ExecutionPolicy Bypass -Command ^
"$ErrorActionPreference='Stop'; ^
function Ensure-Label { param([string]$Name,[string]$Color,[string]$Description); gh label create $Name --color $Color --description $Description 2>$null; if ($LASTEXITCODE -ne 0) { gh label edit $Name --color $Color --description $Description | Out-Null } }; ^
function New-IssueIfNotExists { param([string]$Title,[string]$Priority,[int]$SP,[string]$Depends,[string]$DoD,[string[]]$Labels); $existing = gh issue list --state open --search ('\"' + $Title + '\" in:title') --json title,number | ConvertFrom-Json; if ($existing -and ($existing | Where-Object { $_.title -eq $Title })) { Write-Host ('[skip] exists: ' + $Title); return }; $body = @' ^
## 概要
__TITLE__
...
## 受け入れ条件（DoD）
__DOD__
'@; $body = $body.Replace('__TITLE__',$Title).Replace('__PRIORITY__',$Priority).Replace('__SP__', [string]$SP).Replace('__DEPENDS__',$Depends).Replace('__DOD__',$DoD); $labelArgs=@(); foreach($lb in $Labels){ $labelArgs += @('--label',$lb)}; gh issue create --title $Title --body $body @labelArgs | Out-Null; Write-Host ('[created] ' + $Title) }; ^
$labels = @( ^
...
); foreach ($i in $issues) { New-IssueIfNotExists -Title $i.t -Priority $i.p -SP $i.sp -Depends $i.dep -DoD $i.dod -Labels $i.l }; ^
Write-Host 'Done: labels ensured + issues created/skipped.'; gh issue list --limit 100"
