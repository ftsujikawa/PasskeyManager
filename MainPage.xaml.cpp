...
namespace winrt::PasskeyManager::implementation
{
    ...
    {
        std::wstring operation = L"refresh_snapshot_candidates";
        std::wstring requestId = BuildRequestId(operation);
        ReloadSnapshotCandidates();
        LogInfo(winrt::hstring{ L"sync result=success operation=" + operation + L" request_id=" + requestId });
        co_return;
    }
    ...
    {
        std::wstring operation = L"restore_selected_snapshot";
        std::wstring requestId = BuildRequestId(operation);
        ...
        {
            LogWarning(winrt::hstring{ L"sync result=rejected operation=" + operation + L" reason=candidate_stale request_id=" + requestId });
            co_return;
        }
        ...
        if (auto self = weakThis.get())
        {
            ...
            {
                std::wstring detail =
                    L"sync result=success operation=" + operation + L" source=" +
                    chosen.Source +
                    L" bytes=" +
            ...
            {
                self->LogFailure(winrt::hstring{ L"summary result=failed operation=" + operation + L" step=write_local_snapshot request_id=" + requestId }, hr);
            }
        }
        co_return;
    }
    ...
    winrt::IAsyncAction MainPage::runVaultRecoveryButton_Click(IInspectable const&, RoutedEventArgs const&)
    {
        ...
        if (!pluginEnabled)
        {
            if (auto self{ weakThis.get() })
            {
                ...
                if (launched)
                {
                    self->LogSuccess(winrt::hstring{ L"summary result=success operation=" + operation + L" action=open_settings request_id=" + requestId });
                }
                else
                {
                    self->LogWarning(winrt::hstring{ L"summary result=failed operation=" + operation + L" action=open_settings reason=launch_failed request_id=" + requestId });
                }
            }
            co_return;
        }
        ...
        if (auto self{ weakThis.get() })
        {
            self->LogInfo(winrt::hstring{ L"summary state=ready operation=" + operation + L" stage=precheck_passed next=create_vault_passkey request_id=" + requestId });
        }
        ...
        if (auto self{ weakThis.get() })
        {
            ...
            if (FAILED(hrSetMethod))
            {
                ...
                self->vaultRecoveryHintText().Visibility(Microsoft::UI::Xaml::Visibility::Visible);
                self->LogFailure(winrt::hstring{ L"summary result=failed operation=" + operation + L" step=set_unlock_method_passkey request_id=" + requestId }, hrSetMethod);
                co_return;
            }

            if (FAILED(hrSetSilent))
            {
                self->LogWarning(winrt::hstring{ L"summary result=warning operation=" + operation + L" step=set_silent_off hr=" + std::to_wstring(static_cast<int>(hrSetSilent)) + L" detail=plugin_ui_visibility_unset request_id=" + requestId });
            }

            self->LogInfo(winrt::hstring{ L"summary state=observed operation=" + operation + L" step=create_vault_passkey_returned hr=" + std::to_wstring(static_cast<int>(hrCreatePasskey)) + L" request_id=" + requestId });
            if (FAILED(hrCreatePasskey))
            {
                ...
                self->LogInfo(winrt::hstring{ L"summary state=observed operation=" + operation + L" step=plugin_perform_operation_status hr=" + std::to_wstring(static_cast<int>(pluginPerformStatus)) + L" request_id=" + requestId });
                self->LogInfo(winrt::hstring{ L"summary state=observed operation=" + operation + L" step=plugin_uv_signature_verification_status hr=" + std::to_wstring(static_cast<int>(pluginUvStatus)) + L" request_id=" + requestId });
            ...
            else
            {
                ...
                self->vaultRecoveryHintText().Visibility(Microsoft::UI::Xaml::Visibility::Visible);
                self->LogFailure(winrt::hstring{ L"summary result=failed operation=" + operation + L" step=create_vault_passkey request_id=" + requestId }, hrCreatePasskey);
            }
        }
        co_return;
    }
    ...
    {
        std::wstring operation = L"persist_sync_history_entry";
        ...
        if (FAILED(hr))
        {
            OutputDebugStringW((L"DEBUG: summary result=failed operation=" + operation + L" hr=" + std::to_wstring(static_cast<int>(hr)) + L"\n").c_str());
        }
    }
    ...
    winrt::IAsyncAction MainPage::addAllPluginCredentials_Click(IInspectable const&, RoutedEventArgs const&)
    {
        ...
        if (hr == NTE_EXISTS || hr == HRESULT_FROM_WIN32(ERROR_ALREADY_EXISTS))
        {
            self->LogInfo(winrt::hstring{ BuildSummaryResultLog(L"success", operation, requestId, L"outcome=already_cached_no_new") });
            co_return;
        }
        if (FAILED(hr))
        {
            std::wstring detail = DescribeCredentialOperationFailure(hr);
            self->LogFailure(winrt::hstring{ BuildSummaryResultLog(L"failed", operation, requestId, L"detail=" + detail) }, hr);
            co_return;
        }
        self->LogSuccess(winrt::hstring{ BuildSummaryResultLog(L"success", operation, requestId) });
        co_return;
    ...
    winrt::IAsyncAction MainPage::addSelectedCredentials_Click(IInspectable const&, Microsoft::UI::Xaml::RoutedEventArgs const&)
    {
        ...
        {
            LogWarning(winrt::hstring{ BuildSummaryResultLog(L"rejected", operation, requestId, L"reason=no_selection") });
            co_return;
        }
        ...
        LogInProgress(winrt::hstring{ BuildSummaryStateLog(L"selected", operation, requestId, L"selected=" + std::to_wstring(credentialIdList.size())) });
    ...
    winrt::IAsyncAction MainPage::deleteSelectedPluginCredentialsEverywhere_Click(IInspectable const&, Microsoft::UI::Xaml::RoutedEventArgs const&)
    {
        ...
        if (selectedItems.Size() == 0)
        {
            LogWarning(winrt::hstring{ BuildSummaryResultLog(L"rejected", operation, requestIdToken, L"reason=no_selection") }, E_NOT_SET);
            co_return;
        }
        ...
        LogInProgress(winrt::hstring{ BuildSummaryStateLog(L"running", operation, requestIdToken, L"run=" + std::to_wstring(runId)) });
        ...
        // update the status block with count of selected creds
        hstring statusText = winrt::hstring{ L"INFO: " + BuildSummaryStateLog(L"selected", operation, requestIdToken, L"run=" + std::to_wstring(runId) + L" selected=" + std::to_wstring(credentialIdList.size())) + L"ℹ" };
        ...
        if (hr == HRESULT_FROM_WIN32(ERROR_NOT_FOUND))
        {
            ...
            self->LogInfo(winrt::hstring{ BuildSummaryResultLog(L"not_found", operation, requestIdToken, L"run=" + std::to_wstring(runId) + L" attempts=1 elapsed_ms=" + std::to_wstring(elapsedMs) + L" hr=" + std::to_wstring(static_cast<int>(hr)) + L" selected=" + std::to_wstring(credentialIdList.size()) + L" cached=" + std::to_wstring(cachedCredentialIdList.size()) + L" missing=" + std::to_wstring(missingCount)) });
            co_return;
        }
        if (FAILED(hr))
        {
            auto missingCount = credentialIdList.size() - cachedCredentialIdList.size();
            std::wstring detail = DescribeCredentialOperationFailure(hr);
            self->LogWarning(winrt::hstring{ BuildSummaryResultLog(L"failed", operation, requestIdToken, L"run=" + std::to_wstring(runId) + L" attempts=1 elapsed_ms=" + std::to_wstring(elapsedMs) + L" hr=" + std::to_wstring(static_cast<int>(hr)) + L" selected=" + std::to_wstring(credentialIdList.size()) + L" cached=" + std::to_wstring(cachedCredentialIdList.size()) + L" missing=" + std::to_wstring(missingCount) + L" detail=" + detail) }, hr);
            co_return;
        }
        auto missingCount = credentialIdList.size() - cachedCredentialIdList.size();
        self->LogSuccess(winrt::hstring{ BuildSummaryResultLog(L"success", operation, requestIdToken, L"run=" + std::to_wstring(runId) + L" attempts=1 elapsed_ms=" + std::to_wstring(elapsedMs) + L" hr=" + std::to_wstring(static_cast<int>(hr)) + L" selected=" + std::to_wstring(credentialIdList.size()) + L" cached=" + std::to_wstring(cachedCredentialIdList.size()) + L" missing=" + std::to_wstring(missingCount)) });
        co_return;
    }
    ...
    {
        std::wstring operation = L"copy_logs";
        std::wstring requestId = BuildRequestId(operation);
        if (m_logEntries.empty())
        {
            LogInfo(winrt::hstring{ L"summary result=rejected operation=" + operation + L" reason=no_logs request_id=" + requestId });
            co_return;
        }
        ...
        if (copiedLines > 1)
        {
            LogSuccess(winrt::hstring{ L"summary result=success operation=" + operation + L" scope=visible lines=" + std::to_wstring(copiedLines) + L" request_id=" + requestId });
        }
        else
        {
            LogSuccess(winrt::hstring{ L"summary result=success operation=" + operation + L" scope=visible lines=1 request_id=" + requestId });
        }
        co_return;
    }
    ...
    {
        std::wstring operation = L"delete_all_local_credentials";
        std::wstring requestId = BuildRequestId(operation);
        LogInProgress(winrt::hstring{ L"summary state=running operation=" + operation + L" request_id=" + requestId });
        ...
        if (!resetResult)
        {
            self->LogFailure(winrt::hstring{ L"summary result=failed operation=" + operation + L" detail=credential_store_update_failed request_id=" + requestId }, HRESULT_FROM_WIN32(ERROR_WRITE_FAULT));
            co_return;
        }
        self->LogSuccess(winrt::hstring{ L"summary result=success operation=" + operation + L" request_id=" + requestId });
        co_return;
    ...
    {
        std::wstring operation = L"delete_all_credentials";
        std::wstring requestId = BuildRequestId(operation);
        LogInProgress(winrt::hstring{ L"summary state=running operation=" + operation + L" request_id=" + requestId });
        auto weakThis = get_weak();
        co_await winrt::resume_background();
        ...
        {
            self->LogInfo(winrt::hstring{ L"summary result=success operation=" + operation + L" outcome=cache_not_found_local_deleted request_id=" + requestId });
            co_return;
        }
        if (FAILED(hr) || !resetResult)
        {
            ...
            self->LogFailure(winrt::hstring{ L"summary result=failed operation=" + operation + L" detail=" + detail + L" request_id=" + requestId }, effectiveHr);
            co_return;
        }
        self->LogSuccess(winrt::hstring{ L"summary result=success operation=" + operation + L" request_id=" + requestId });
    }
    ...
    winrt::IAsyncAction MainPage::activatePluginButton_Click(IInspectable const& sender, Microsoft::UI::Xaml::RoutedEventArgs const& e)
    {
        ...
        auto weakThis = get_weak();
        std::wstring operation = L"activate_plugin";
        std::wstring requestId = BuildRequestId(operation);
        LogInProgress(winrt::hstring{ L"summary state=running operation=" + operation + L" action=open_settings request_id=" + requestId });
        auto uri = Windows::Foundation::Uri(L"ms-settings:passkeys-advancedoptions");
        bool launched = co_await Windows::System::Launcher::LaunchUriAsync(uri);
        if (!launched)
        {
            LogWarning(winrt::hstring{ L"summary result=failed operation=" + operation + L" action=open_settings reason=launch_failed request_id=" + requestId });
            co_return;
        }

        LogSuccess(winrt::hstring{ L"summary result=success operation=" + operation + L" action=open_settings request_id=" + requestId });
        ...
        for (int attempt = 0; attempt < 5; ++attempt)
        {
            ...
            if (auto self = weakThis.get())
            {
                ...
                {
                    self->LogSuccess(winrt::hstring{ L"summary result=success operation=" + operation + L" outcome=state_enabled request_id=" + requestId });
                    co_return;
                }
            }
            else
        ...
        LogWarning(winrt::hstring{ L"summary result=warning operation=" + operation + L" outcome=state_not_enabled_after_wait request_id=" + requestId });
        co_return;
    }
    ...
    {
        std::wstring operation = L"set_vault_lock_state";
        std::wstring requestId = BuildRequestId(operation);
