...
- `check_sync_runtime_log_keys.cmd`  
  実運用ログ向けの軽量チェック（必須異常シナリオを要求しない）。現在は12ルールを検証し、`manual_resync` success summary の `request_id` フォーマット整合と `delete_selected_credentials_everywhere` success summary の `request_id` 必須を含む
