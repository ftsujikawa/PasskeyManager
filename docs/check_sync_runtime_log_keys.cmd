...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$deleteEverywhereSuccessSummaryLines = @(); foreach ($line in $lines) { if (![string]::IsNullOrWhiteSpace($line) -and $line -match '^SUCCESS:\s+summary\s+result=success\s+operation=delete_selected_credentials_everywhere\b') { $deleteEverywhereSuccessSummaryLines += $line } }" ^
  "$deleteEverywhereSummaryRequestIdMissing = @(); foreach ($line in $deleteEverywhereSuccessSummaryLines) { if ($line -notmatch '(^|\s)request_id=') { $deleteEverywhereSummaryRequestIdMissing += $line } }" ^
  "if ($deleteEverywhereSummaryRequestIdMissing.Count -eq 0) { Write-Host 'PASS: request_id_with_delete_selected_credentials_everywhere_summary_success' } else { Write-Host ('FAIL: request_id_with_delete_selected_credentials_everywhere_summary_success count=' + $deleteEverywhereSummaryRequestIdMissing.Count); $failed += 'request_id_with_delete_selected_credentials_everywhere_summary_success' }" ^
