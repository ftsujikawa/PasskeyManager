...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$delSum = @(); foreach ($line in $lines) { if (![string]::IsNullOrWhiteSpace($line) -and $line -match '^SUCCESS:\s+summary\s+result=success\s+operation=delete_selected_credentials_everywhere\b') { $delSum += $line } }" ^
  "$delReqMiss = @(); foreach ($line in $delSum) { if ($line -notmatch '(^|\s)request_id=') { $delReqMiss += $line } }" ^
  "if ($delReqMiss.Count -eq 0) { Write-Host 'PASS: request_id_with_delete_everywhere_summary_success' } else { Write-Host ('FAIL: request_id_with_delete_everywhere_summary_success count=' + $delReqMiss.Count); $failed += 'request_id_with_delete_everywhere_summary_success' }" ^
  "$setOk = @(); foreach ($line in $lines) { if (![string]::IsNullOrWhiteSpace($line) -and $line -match '^(INFO|SUCCESS):\s+sync\s+result=success\s+operation=(load_settings|save_settings)\b') { $setOk += $line } }" ^
  "$setReqMiss = @(); foreach ($line in $setOk) { if ($line -notmatch '(^|\s)request_id=') { $setReqMiss += $line } }" ^
  "if ($setReqMiss.Count -eq 0) { Write-Host 'PASS: request_id_with_settings_sync_success' } else { Write-Host ('FAIL: request_id_with_settings_sync_success count=' + $setReqMiss.Count); $failed += 'request_id_with_settings_sync_success' }" ^
  "$manualResyncSummaryRequestIdInvalid = @(); foreach ($line in $manualResyncSuccessSummaryLines) { if ($line -match '(^|\s)request_id=([^\s]+)') { $rawValue = $matches[2]; $value = & $normalizeToken $rawValue; if ($value -notmatch '^[0-9]{8}T[0-9]{9}Z-manual_resync$') { $manualResyncSummaryRequestIdInvalid += $line } } }" ^
  "if ($manualResyncSummaryRequestIdInvalid.Count -eq 0) { Write-Host 'PASS: request_id_format_with_manual_resync_summary_success' } else { Write-Host ('FAIL: request_id_format_with_manual_resync_summary_success count=' + $manualResyncSummaryRequestIdInvalid.Count); $failed += 'request_id_format_with_manual_resync_summary_success' }" ^
