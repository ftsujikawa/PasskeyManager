...
### ケース8: Credential操作エラー導線
1. 何も選択せずに Selected 系メニューを押す
2. 期待結果:
   - `No credentials selected` 警告が表示される
3. 一覧更新直後に古い選択で add/delete を実行して対象不一致を再現する
4. 期待結果:
   - `No matching credentials found...` を含む失敗ログが表示される
